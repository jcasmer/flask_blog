...
from Flask_blog.users.forms import RegistrationForm, LoginForm, UpdateAccoutForm, RequestResetForm, ResetPasswordForm
...
def user_posts(username):
    ...
    return render_template('templates/user_post.html', user=user, posts=posts)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('templates/reset_request.html', title='Reset Password', form=form)
...
def reset_token(token):
    ...
    if not user:
        flash('That is an invalidad or expired token', 'warning')
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('templates/reset_token.html', title='Reset Password', form=form)
